---
doc: 07_FRONTEND_IMPLEMENTATION_SVELTE
status: ready
version: 1
created: 2026-04-18
scope: SvelteKit + TypeScript implementation patterns — bootstrap, stores, fetch wrapper, SSE client, Zod schemas, Svelte Flow, route tree
relates_to:
  - 03_ARCHITECTURE
...
**Stack:** SvelteKit · TypeScript · Svelte Flow (@xyflow/svelte) · Melt UI · SuperForms · Zod · svelte-sonner · Tailwind CSS

> This is the SvelteKit implementation. The React equivalent is in 07_FRONTEND_IMPLEMENTATION.md.
> Both share the same backend. Only one is deployed to production at a time (controlled by DEPLOY_FRONTEND).
> Where sections below say "identical to the React version", copy from 07_FRONTEND_IMPLEMENTATION.md and apply only the listed substitutions.
...
export default defineConfig({
  ...
  server: {
    ...
    port: 3001,         // dev port — React dev uses 3000; both run simultaneously in local dev
...
public env vars (prefix `PUBLIC_`). The React version's `VITE_API_URL` is named `PUBLIC_API_URL` here.
...
// Prod → ''  (same-origin; nginx or ALB proxies /api/ and /auth/ to api container)
```
...
The backend `.env.example` uses `FRONTEND_URLS` (a list) to support both dev frontend ports simultaneously: `["http://localhost:3000","http://localhost:3001"]`.
...
static files served by nginx. The production image runs Node directly on port 80 (matching
the ALB/Nginx target group — same as the React nginx container).
...
RUN npm ci
COPY . .
# PUBLIC_API_URL is intentionally empty — resolved at runtime via environment variable
# set by Railway/Vercel/ALB, not baked into the image
ENV PUBLIC_API_URL=""
RUN npm run build
...
**Key differences from the React/nginx Dockerfile:**
- Production serves on port 80 via Node (not nginx serving static files)
- Dev stage uses port 3001 (React dev uses 3000 — both run simultaneously in local dev)
- `PUBLIC_API_URL` is not baked in — set it as an environment variable at deploy time in Railway/Vercel/ALB
...
```yaml
# .github/workflows/ci.yaml — frontend job (updated for SvelteKit)
frontend:
  steps:
...
- name: Build and push web image
  run: |
    docker build --target production \
      ...
      -t $ECR_REGISTRY/idealens-web-svelte:$GITHUB_SHA infra/Dockerfile.frontend
